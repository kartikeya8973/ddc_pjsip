...
void legacy_tcp(void)
{
	...
	server_addr.sin_port = htons(port_s);			 // port we have defined
	server_addr.sin_addr.s_addr = inet_addr(ip_s); // local ip address

	// Binding the newly created socket to the given IPP address
	n_s = bind(server_sock, (struct sockaddr *)&server_addr, sizeof(server_addr));
	if (n_s < 0)
	{
	...
	while (1)
	{
		...
		addr_size_s = sizeof(client_addr);
		client_sock = accept(server_sock, (struct sockaddr *)&client_addr, &addr_size_s);
		printf("[+]Client connected...\n");

		// Clearing the buffer
		bzero(menuin, 80);
		// Receive message from client
		recv(client_sock, menuin, sizeof(menuin), 0);
		printf("Client: %s\n", menuin);
		...
		// Clearing the menuin
		bzero(menuin, 80);
		strcpy(menuin, "Hi this is the server\n");
		printf("Server: %s\n", menuin);
		// Sending message to the client
		send(client_sock, menuin, strlen(menuin), 0);
...
void *tcp_server_thread(void)
{
	...
	// assigning port and IP
	server_addr.sin_family = AF_INET;			 // this means IPv4
	server_addr.sin_port = htons(port);			 // port we have defined
